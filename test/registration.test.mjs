// Tool-surface invariants for @three-ws/audio-mcp.
//
...
//
// Run: node --test packages/audio-mcp/test/registration.test.mjs
...
test('every tool is a read-only, live-data transform', () => {
	for (const tool of TOOLS) {
		// These tools synthesize/recognize audio or read the clip library — they
		// transform input or read live data, never mutating platform state.
		...
		// Live synthesis/recognition/library reads are never idempotent — the
		// audio bytes, transcript, and catalog move between calls.
		assert.equal(tool.annotations.idempotentHint, false, `${tool.name} is not idempotent`);
	}
});
